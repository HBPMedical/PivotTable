...
query := 'CREATE TABLE ' || table_name || '( id integer, pid integer,' || query || '@);';
query := regexp_replace(query, ',@', '');
execute format('DROP TABLE IF EXISTS ' || table_name);
execute format(query);
table_name := 'new_table';
for id, pid, startdate, concept, valtype, char_val, num_val in 
	select encounter_num, patient_num, start_date, concept_cd, valtype_cd, tval_char, nval_num 
	from observation_fact
loop
currentid := ',' || id || ',';
...
--	raise notice 'Value: %', 'insert into ' || table_name  || '( id, ' || '"' || concept || '"' || ') VALUES (' || id || ',''' ||  char_val || ''')';
		execute format('insert into ' || table_name  || '( id, pid, ' || '"' || concept || '"' || ') VALUES (' || id || ',' || pid || ',''' ||  char_val || ''')');
		usedid := usedid || ',' || id || ',';
		end if;
	end if;
    ELSE 
...
--	raise notice 'Value: %', 'insert into ' || table_name  || '( id, ' || '"' || concept || '"' || ') VALUES (' || id || ')';
		execute format('insert into ' || table_name  || '( id, pid, ' || '"' || concept || '"' || ') VALUES (' || id || ',' || pid || ',' ||  num_val || ')');
		usedid := usedid || ',' || id ||',';
		end if;
	end if;
END case;
...
COPY (Select * From new_table) To '/tmp/PPMI_flattened.csv' With CSV DELIMITER ',' HEADER;
--execute format('DROP TABLE IF EXISTS ' || table_name);
end;$$
