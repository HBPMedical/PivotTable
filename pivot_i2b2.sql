...
CASE
	...
	IF coalesce(TRIM(char_val), '''') <> '''' AND char_val IS NOT NULL THEN
		...
		ELSE
		execute format(''insert into '' || table_name  || ''( pid, subjectageyears, subjectagemonths, gender, dataset, agegroup, '' || ''"'' || concept || ''"'' || '') VALUES ('' || pid || '','' || subjectageyears || '','' || subjectagemonths || '','''''' || gender || '''''','''''' || dataset || '''''','''''' || agegroup || '''''','''''' ||  char_val || '''''')'');
		usedid := usedid || '','' || pid || '','';
		END IF;
	END IF;
    ELSE 
	IF num_val IS NOT NULL THEN
		...
		ELSE
		execute format(''INSERT INTO '' || table_name  || ''( pid, subjectageyears, subjectagemonths, gender, dataset, agegroup, '' || ''"'' || concept || ''"'' || '') VALUES ('' || pid || '','' || subjectageyears || '','' || subjectagemonths || '','''''' || gender || '''''','''''' || dataset || '''''','''''' || agegroup || '''''','''''' ||  num_val || '''''')'');
		usedid := usedid || '','' || pid ||'','';
		END IF;
	END IF;
END CASE;
