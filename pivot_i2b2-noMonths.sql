...
CASE
	...
	IF coalesce(TRIM(char_val), '''') <> '''' and char_val is not null then
		...
		execute format(''insert into '' || table_name  || ''( pid, subjectageyears, gender, dataset, '' || ''"'' || concept || ''"'' || '') VALUES ('' || pid || '','' || subjectageyears || '','''''' || gender || '''''','''''' || dataset || '''''','''''' ||  char_val || '''''')'');
		usedid := usedid || '','' || pid || '','';
		end if;
	end if;
    ELSE 
	if num_val is not null then
		...
		execute format(''insert into '' || table_name  || ''( pid, subjectageyears, gender, dataset,  '' || ''"'' || concept || ''"'' || '') VALUES ('' || pid || '','' || subjectageyears || '','''''' || gender || '''''','''''' || dataset || '''''','''''' ||  num_val || '''''')'');
		usedid := usedid || '','' || pid ||'','';
		end if;
	end if;
END case;
